...
3. Expand a command row to show output and inspect the output panel border contrast.
4. Confirm status colors for running/success/error command rows are distinguishable in dark mode.
5. Toggle back to `Light` theme and confirm command rows still use the existing light styling.
